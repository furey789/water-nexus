...
$( document ).ready(function() {
  ...
  if ($("#graph").length > 0) {
    ...
    function createGraphics(data2,colors,DateMapIndex) {
    	...
    	function findMaxY(){
    		var max=1; // initial value for plot
    	...
    	function findMinY(){
    		var min=0; // initial value for plot
    		for(var i=0; i < data2.length; i++) {
    			if(data2[i].vis=="1"){
    				var fundData=data2[i].priceList;
    				for(var j=0;j<fundData.length;j++){
      ...
      // function findNumDataCols(xPosition){
      //   //console.log("MOO:", Object.keys(data2), data2["0"]["priceList"][0])
      ...
      // findNumDataCols();
    	...
    	var handleMouseOverGraph = function(event) {
    		var mouseX = event.pageX-190;  // controls mouseX relative to vert line
    	...
    	.attr("height", height);
      // console.log(data2[0].priceList[data2[0].priceList.length-1].date);
      ...
      // COMMENTED OUT - to remove line-number axis on yellow bar
    	// context.append("g")
    	//     .attr("class", "x axis")
      ...
      //add text to explain yellow bar
      context.append("text")
        .attr("class", "area")
        .attr("x", function(d) { return 0; })
        .attr("y", function(d) { return height2+25; })
        .text( function (d) { return "(click and drag on yellow bar to zoom in on plot)"; })
        .attr("font-family", "sans-serif")
        .attr("font-size", "6px")
        .attr("fill", "black");
